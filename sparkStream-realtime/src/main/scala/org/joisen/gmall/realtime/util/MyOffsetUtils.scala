...
object MyOffsetUtils {
  ...
  def saveOffset(topic: String, groupId: String, offsetRanges: Array[OffsetRange]): Unit ={
    if(offsetRanges != null && offsetRanges.length > 0){
      val offsets: java.util.HashMap[String, String] = new java.util.HashMap[String,String]
  ...
  def readOffset(topic: String, groupId: String): Map[TopicPartition, Long] ={
    ...
    val offsets: util.Map[String, String] = jedis.hgetAll(redisKey) // 读取redis中存储的offset
