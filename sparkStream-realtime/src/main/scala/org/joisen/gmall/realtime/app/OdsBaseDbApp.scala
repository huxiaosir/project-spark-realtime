...
import org.joisen.gmall.realtime.util.{MyKafkaUtils, MyOffsetUtils, MyRedisUtils}
...
object OdsBaseDbApp {
  def main(args: Array[String]): Unit = {
    ...
    var kafkaDStream: InputDStream[ConsumerRecord[String, String]] = null
      if(offsets.nonEmpty && offsets != null){
        println("redis: " + offsets)
        ...
        rdd.foreachPartition(
          jsonObjIter => {
            ...
            for(jsonObj <- jsonObjIter){
              ...
              if (opValue != null){ // opValue 不为空再进行分流
                // 提取表名
                ...
                if(dimTablesBC.value.contains(tableName)){
                  // 维度数据
                  // 写入redis中选择什么类型：string  list  set  zset  hash
